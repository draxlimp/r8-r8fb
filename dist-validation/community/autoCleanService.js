...
exports.AutoCleanService = void 0;
...
class AutoCleanService {
    ...
    matches(message, rule) {
        if (rule.ignorePinned && message.pinned)
            return false;
        if (message.webhookId && !rule.includeWebhooks)
            return false;
        if (message.author?.bot && !message.webhookId && !rule.includeBots)
            return false;
        ...
        if (rule.mode === 'all')
            return true;
        if (rule.mode === 'images') {
            const hasImageAttachment = [...(message.attachments?.values?.() ?? [])].some((entry) => String(entry.contentType ?? '').startsWith('image/') || /\.(png|jpe?g|webp|gif)$/i.test(String(entry.name ?? '')));
            return hasImageAttachment || IMAGE_LINK_PATTERN.test(content);
        }
        if (rule.mode === 'text')
            return Boolean(content) && attachmentCount === 0;
        return LINK_PATTERN.test(content);
    }
    schedule(message, config, rule) {
        const delay = Math.max(5, Math.min(2_592_000, rule.delaySeconds)) * 1000;
        const timer = setTimeout(async () => {
            try {
                if (message.deletable === false)
                    throw new Error('A mensagem não pode ser apagada pelo bot.');
...
exports.AutoCleanService = AutoCleanService;
//# sourceMappingURL=autoCleanService.js.map