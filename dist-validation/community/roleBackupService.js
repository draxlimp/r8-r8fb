...
exports.RoleBackupService = void 0;
...
class RoleBackupService {
    async create(guild, actorId, config) {
        const current = config ?? await guildConfigStore_1.guildConfigStore.get(guild.id);
        ...
        const backup = {
            id: `ROLE-${new Date().toISOString().replace(/[-:TZ.]/g, '').slice(0, 14)}-${(0, ids_1.randomId)(3)}`,
            createdBy: actorId,
        ...
        await guildConfigStore_1.guildConfigStore.set(guild.id, current);
        return backup;
    }
    async restoreLatest(guild, actorId, config) {
        const current = config ?? await guildConfigStore_1.guildConfigStore.get(guild.id);
        const backup = current.community.roleBackups.at(-1);
        if (!backup)
            throw new Error('Nenhum backup de cargos foi criado');
    ...
    commandPayload(userId, config) {
        ...
        const embed = new discord_js_1.EmbedBuilder()
            ...
            .setColor(0x111111)
            .setTimestamp();
        const row = new discord_js_1.ActionRowBuilder().addComponents(new discord_js_1.ButtonBuilder().setCustomId(`rolebackup|${userId}|create`).setLabel('Criar backup').setStyle(discord_js_1.ButtonStyle.Primary), new discord_js_1.ButtonBuilder().setCustomId(`rolebackup|${userId}|restore`).setLabel('Restaurar último').setStyle(discord_js_1.ButtonStyle.Danger).setDisabled(!latest), new discord_js_1.ButtonBuilder().setCustomId(`rolebackup|${userId}|close`).setEmoji(emojis_1.UI_EMOJIS.close).setStyle(discord_js_1.ButtonStyle.Secondary));
        return { embeds: [embed], components: [row], allowedMentions: { parse: [] } };
    }
    async handleInteraction(interaction) {
        const customId = typeof interaction.customId === 'string' ? interaction.customId : '';
        if (!customId.startsWith('rolebackup|'))
            return false;
        const [, ownerId, action] = customId.split('|');
        if (interaction.user.id !== ownerId) {
            await interaction.reply({ content: 'Este painel pertence a outro usuário.', flags: discord_js_1.MessageFlags.Ephemeral });
            return true;
        }
        if (!interaction.guild || !interaction.member?.permissions?.has(discord_js_1.PermissionFlagsBits.ManageRoles)) {
            await interaction.reply({ content: 'Você precisa da permissão Gerenciar Cargos.', flags: discord_js_1.MessageFlags.Ephemeral });
            return true;
        }
        if (action === 'close') {
        ...
        if (action === 'create')
            await this.create(interaction.guild, interaction.user.id, config);
        else if (action === 'restore')
            await this.restoreLatest(interaction.guild, interaction.user.id, config);
        else
            return false;
        const refreshed = await guildConfigStore_1.guildConfigStore.get(interaction.guild.id);
        await interaction.editReply(this.commandPayload(interaction.user.id, refreshed));
        return true;
    }
}
exports.RoleBackupService = RoleBackupService;
//# sourceMappingURL=roleBackupService.js.map