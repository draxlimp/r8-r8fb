...
exports.createTwitterCard = createTwitterCard;
const canvas_1 = require("@napi-rs/canvas");
const WIDTH = 1200;
const PADDING = 68;
const CONTENT_WIDTH = WIDTH - PADDING * 2;
async function createTwitterCard(input) {
    const mediaEntries = await Promise.all(input.mediaUrls.slice(0, 4).map(async (url) => ({ url, image: await safeLoad(url) })));
    const loadedMedia = mediaEntries.filter((entry) => Boolean(entry.image));
    const measuring = (0, canvas_1.createCanvas)(WIDTH, 400).getContext('2d');
...
function drawMediaGrid(ctx, images, x, y, width, height) {
    const gap = 8;
    ctx.save();
...
//# sourceMappingURL=twitterCanvas.js.map