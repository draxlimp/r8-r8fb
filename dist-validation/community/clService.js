...
exports.handleClearCommand = handleClearCommand;
...
async function handleClearCommand(message, app) {
    ...
    const hasConfiguredRole = message.member.roles.cache.some((role) => config.community.cl.allowedRoleIds.includes(role.id));
    const hasManageMessages = config.community.cl.allowManageMessages && message.member.permissions.has(discord_js_1.PermissionFlagsBits.ManageMessages);
    ...
    const status = await message.channel.send(`${emojis_1.UI_LOADING_MENTION} Limpeza iniciada para **${target.user.username}**. Aguarde a conclusão.`);
    try {
        const result = await clearUserMessages(message.channel, targetId, config.community.cl.scanLimit, status.id);
        if (config.community.cl.deleteCommandMessage && message.deletable)
        ...
        logger_1.logger.error('Falha no comando CL.', { guildId: message.guild.id, channelId: message.channel.id, error: String(error) });
...
async function clearUserMessages(channel, targetId, scanLimit, excludedMessageId) {
    let before;
    let scanned = 0;
    let matched = 0;
    let deleted = 0;
    ...
    while (scanned < limit) {
        ...
        const matching = [...batch.values()].filter((item) => item.author?.id === targetId && item.id !== excludedMessageId);
        matched += matching.length;
        const recent = matching.filter((item) => Date.now() - item.createdTimestamp < TWO_WEEKS_MS);
        const old = matching.filter((item) => Date.now() - item.createdTimestamp >= TWO_WEEKS_MS);
...
async function temporaryReply(message, content) {
    ...
    if (sent)
        setTimeout(() => void sent.delete().catch(() => undefined), 5_000);
}
//# sourceMappingURL=clService.js.map