...
exports.createWantedCard = createWantedCard;
exports.createJailCard = createJailCard;
exports.createProfileCard = createProfileCard;
exports.createQuoteCard = createQuoteCard;
exports.createAvatarEffectCard = createAvatarEffectCard;
exports.createAchievementCard = createAchievementCard;
const canvas_1 = require("@napi-rs/canvas");
async function createWantedCard(user) {
    const canvas = (0, canvas_1.createCanvas)(900, 1120);
...
async function createJailCard(user, guildName = 'Comunidade') {
    const canvas = (0, canvas_1.createCanvas)(1200, 820);
    const ctx = canvas.getContext('2d');
    ...
    for (let index = 0; index < 6; index += 1)
        ctx.fillText(`${170 - index * 10} cm`, 175, 160 + index * 82);
    ...
    for (let y = 430; y < 670; y += 58)
        ctx.fillRect(365, y, 470, 22);
    ctx.restore();
    const image = user.avatarUrl ? await safeLoad(user.avatarUrl) : null;
...
async function createProfileCard(user, details) {
    const canvas = (0, canvas_1.createCanvas)(1100, 560);
    const ctx = canvas.getContext('2d');
    ...
    for (let x = -100; x < 1200; x += 130) {
        ctx.beginPath();
        ctx.arc(x, 40 + (x % 260), 80, 0, Math.PI * 2);
        ctx.fill();
    }
    ctx.fillStyle = 'rgba(17,24,39,0.86)';
    roundRect(ctx, 40, 40, 1020, 480, 34);
    ctx.fill();
    const image = user.avatarUrl ? await safeLoad(user.avatarUrl) : null;
    ...
    ctx.fillText(trim(user.displayName, 27), 410, 165);
    ctx.fillStyle = '#aeb8d0';
    ctx.font = '600 25px Arial';
    ctx.fillText(`@${trim(user.username, 34)}`, 410, 205);
    ctx.fillStyle = '#8ea1ff';
    ctx.font = '800 21px Arial';
    ctx.fillText(details.guildName.slice(0, 48), 410, 260);
    drawInfoBox(ctx, 410, 300, 270, 105, 'ENTROU', details.joinedText);
    drawInfoBox(ctx, 710, 300, 270, 105, 'CARGOS', String(details.roleCount));
    ctx.fillStyle = '#77829c';
    ctx.font = '500 18px Arial';
    ctx.fillText(`ID ${user.id}`, 410, 466);
    return Buffer.from(await canvas.encode('png'));
}
async function createQuoteCard(user, text, guildName) {
    const canvas = (0, canvas_1.createCanvas)(1200, 650);
    const ctx = canvas.getContext('2d');
    ...
    overlay.addColorStop(1, 'rgba(15,23,42,0.98)');
    ctx.fillStyle = overlay;
    ctx.fillRect(0, 0, 1200, 650);
    ctx.fillStyle = '#60a5fa';
    ctx.font = '900 100px Georgia';
    ctx.textAlign = 'left';
    ctx.fillText('“', 590, 155);
    ctx.fillStyle = '#ffffff';
    ctx.font = '700 34px Arial';
    drawWrappedText(ctx, text.slice(0, 500), 610, 205, 500, 48, 7);
    ctx.fillStyle = '#93c5fd';
    ctx.font = '800 25px Arial';
    ctx.fillText(`— ${trim(user.displayName, 28)}`, 610, 535);
    ctx.fillStyle = '#94a3b8';
    ctx.font = '500 18px Arial';
    ctx.fillText(guildName.slice(0, 55), 610, 572);
    return Buffer.from(await canvas.encode('png'));
}
async function createAvatarEffectCard(user, effect) {
    const canvas = (0, canvas_1.createCanvas)(900, 900);
    const ctx = canvas.getContext('2d');
    ...
    if (!image) {
        ...
        ctx.fillRect(80, 80, 740, 740);
        ctx.fillStyle = '#fff';
        ctx.font = '900 220px Arial';
        ctx.textAlign = 'center';
        ctx.textBaseline = 'middle';
        ctx.fillText(initials(user.displayName), 450, 450);
    }
    else if (effect === 'pixelate') {
        const temp = (0, canvas_1.createCanvas)(48, 48);
        const tctx = temp.getContext('2d');
        drawCover(tctx, image, 0, 0, 48, 48);
        ctx.imageSmoothingEnabled = false;
        ctx.drawImage(temp, 80, 80, 740, 740);
        ...
        if (effect === 'grayscale' || effect === 'invert') {
            ...
            for (let index = 0; index < data.data.length; index += 4) {
                ...
                if (effect === 'grayscale') {
                    ...
                    data.data[index] = 255 - r;
                    data.data[index + 1] = 255 - g;
                    data.data[index + 2] = 255 - b;
                }
            }
            ctx.putImageData(data, 80, 80);
        }
        else if (effect === 'blur') {
            const copy = (0, canvas_1.createCanvas)(740, 740);
            const copyCtx = copy.getContext('2d');
            copyCtx.drawImage(canvas, 80, 80, 740, 740, 0, 0, 740, 740);
            ctx.fillStyle = '#101318';
            ctx.fillRect(80, 80, 740, 740);
            ctx.globalAlpha = 0.085;
            for (let dx = -16; dx <= 16; dx += 4)
                for (let dy = -16; dy <= 16; dy += 4)
                    ctx.drawImage(copy, 80 + dx, 80 + dy);
            ctx.globalAlpha = 1;
        }
    }
    ctx.strokeStyle = '#ffffff';
    ctx.lineWidth = 12;
    ctx.strokeRect(80, 80, 740, 740);
    ctx.fillStyle = 'rgba(0,0,0,0.72)';
    ctx.fillRect(80, 760, 740, 60);
    ctx.fillStyle = '#ffffff';
    ctx.font = '800 27px Arial';
    ctx.textAlign = 'center';
    ctx.textBaseline = 'middle';
    ctx.fillText(`${effectLabel(effect)} • @${trim(user.username, 25)}`, 450, 790);
    return Buffer.from(await canvas.encode('png'));
}
async function createAchievementCard(user, achievement) {
    const canvas = (0, canvas_1.createCanvas)(1100, 420);
    const ctx = canvas.getContext('2d');
    ...
    roundRect(ctx, 34, 34, 1032, 352, 30);
    ctx.fill();
    ctx.fillStyle = '#161b24';
    roundRect(ctx, 46, 46, 1008, 328, 24);
    ctx.fill();
    const image = user.avatarUrl ? await safeLoad(user.avatarUrl) : null;
    ...
    ctx.fillText('CONQUISTA DESBLOQUEADA', 360, 116);
    ctx.fillStyle = '#ffffff';
    ctx.font = '900 39px Arial';
    drawWrappedText(ctx, achievement.slice(0, 150), 360, 170, 620, 48, 3);
    ctx.fillStyle = '#9ca3af';
    ctx.font = '600 21px Arial';
    ctx.fillText(`${user.displayName} • @${user.username}`, 360, 330);
    return Buffer.from(await canvas.encode('png'));
}
async function safeLoad(url) { try {
    return await (0, canvas_1.loadImage)(url);
}
catch {
    return null;
} }
function drawHead(ctx, image, x, y, size, fallback) {
    ctx.save();
    ctx.beginPath();
...
function drawSquareImage(ctx, image, x, y, size, fallback, fallbackColor) {
    ctx.save();
    ctx.beginPath();
...
function drawCircleImage(ctx, image, x, y, size, fallback, fallbackColor) {
    ctx.save();
    ctx.beginPath();
    ctx.arc(x + size / 2, y + size / 2, size / 2, 0, Math.PI * 2);
    ctx.clip();
...
function drawInfoBox(ctx, x, y, width, height, label, value) {
    ...
    ctx.fillStyle = '#ffffff';
    ctx.font = '800 25px Arial';
    ctx.fillText(value.slice(0, 20), x + 22, y + 72);
}
...
function effectLabel(effect) { return { blur: 'Desfoque', pixelate: 'Pixelado', grayscale: 'Preto e branco', invert: 'Cores invertidas' }[effect]; }
function initials(value) { return value.split(/\s+/).filter(Boolean).slice(0, 2).map(part => part[0]?.toUpperCase() ?? '').join('') || '?'; }
function trim(value, maximum) { return value.length > maximum ? `${value.slice(0, maximum - 1)}…` : value; }
...
//# sourceMappingURL=funCanvas.js.map