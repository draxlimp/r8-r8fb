...
function tutorialPage(session, ids, config) {
    ...
    const container = (0, common_1.baseContainer)(config.panel.color, 'Tutorial', 'Guia de configuração e uso das funções do servidor.');
    const sectionMenu = new discord_js_1.StringSelectMenuBuilder()
        ...
        .addOptions(...Object.entries(TUTORIALS).map(([value, item]) => new discord_js_1.StringSelectMenuOptionBuilder().setLabel(item.label).setDescription(item.description).setValue(value).setEmoji(emojis_1.TUTORIAL_SECTION_EMOJIS[value] ?? emojis_1.UI_EMOJIS.tutorial).setDefault(value === sectionKey)));
    const entryMenu = new discord_js_1.StringSelectMenuBuilder()
        ...
        .setMinValues(1).setMaxValues(1)
        .addOptions(...Object.entries(section.entries).map(([value, item]) => new discord_js_1.StringSelectMenuOptionBuilder().setLabel(item.label).setDescription(item.summary).setValue(value).setEmoji(emojis_1.TUTORIAL_ENTRY_EMOJIS[value] ?? emojis_1.UI_EMOJIS.more).setDefault(value === entryKey)));
    container.addActionRowComponents(new discord_js_1.ActionRowBuilder().addComponents(sectionMenu), new discord_js_1.ActionRowBuilder().addComponents(entryMenu));
    container.addTextDisplayComponents(new discord_js_1.TextDisplayBuilder().setContent(`### ${entry.label}\n${entry.summary}\n\n${entry.body}`));
