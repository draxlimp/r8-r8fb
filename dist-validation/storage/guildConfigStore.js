...
exports.guildConfigStore = void 0;
const node_path_1 = __importDefault(require("node:path"));
const defaults_1 = require("../config/defaults");
const jsonStore_1 = require("./jsonStore");
function migrateTicketPanel(panel, position) {
    ...
    const base = {
        ...
        external: { ...(0, defaults_1.defaultTicketAppearance)('external'), ...(panel.external ?? {}) },
        internal: { ...(0, defaults_1.defaultTicketAppearance)('internal'), ...(panel.internal ?? {}) },
...
function migrateRolePanel(panel, position) {
    const base = (0, defaults_1.defaultRolePanel)(typeof panel.createdBy === 'string' ? panel.createdBy : 'migration', position);
...
function migrateApplicationForm(form, position) {
    const base = (0, defaults_1.defaultApplicationForm)(typeof form.createdBy === 'string' ? form.createdBy : 'migration', position);
    return {
        ...base,
        ...form,
...
function cleanLegacyAppearance(appearance) {
    if (typeof appearance.footer === 'string' && /r8fb|desenvolvido por/i.test(appearance.footer))
        appearance.footer = '';
    if (typeof appearance.separator === 'string' && /^[\s─—_-]+$/.test(appearance.separator))
        appearance.separator = '';
    return appearance;
}
function migrate(value, guildId) {
    const base = (0, defaults_1.createDefaultGuildConfig)(guildId);
    if (!value || typeof value !== 'object')
        return base;
    const raw = value;
    const rawCommunity = raw.community;
    const rawTelloyn = rawCommunity?.telloyn ?? rawCommunity?.telolin;
    const merged = {
        ...base,
        ...raw,
        guildId,
        ...
        community: {
            ...
            welcome: {
                ...
                appearance: { ...(0, defaults_1.defaultMessageAppearance)('welcome'), ...(rawCommunity?.welcome?.appearance ?? {}) }
            },
            goodbye: {
                ...base.community.goodbye,
                ...rawCommunity?.goodbye,
                appearance: { ...(0, defaults_1.defaultMessageAppearance)('goodbye'), ...(rawCommunity?.goodbye?.appearance ?? {}) }
            },
            suggestions: { ...base.community.suggestions, ...rawCommunity?.suggestions },
            telloyn: {
                ...base.community.telloyn,
                ...rawTelloyn,
                appearance: { ...(0, defaults_1.defaultTelloynAppearance)(), ...(rawTelloyn?.appearance ?? {}) }
            },
            instagram: {
                ...base.community.instagram,
                ...rawCommunity?.instagram,
                appearance: { ...(0, defaults_1.defaultInstagramAppearance)(), ...(rawCommunity?.instagram?.appearance ?? {}) },
            ...
            autoClean: {
                ...
                rules: Array.isArray(rawCommunity?.autoClean?.rules)
                    ? rawCommunity.autoClean.rules.slice(0, 10).map((rule, index) => {
                        const now = new Date().toISOString();
            ...
            reputation: {
                scores: { ...base.community.reputation.scores, ...(rawCommunity?.reputation?.scores ?? {}) },
                lastGivenAt: { ...base.community.reputation.lastGivenAt, ...(rawCommunity?.reputation?.lastGivenAt ?? {}) }
            },
    ...
    for (const event of defaults_1.LOG_EVENTS)
        merged.logs.events[event] = { ...(0, defaults_1.defaultLogEvent)(), ...merged.logs.events[event] };
    for (const command of defaults_1.COMMAND_NAMES) {
        merged.commands.aliases[command] = Array.isArray(merged.commands.aliases[command]) ? merged.commands.aliases[command].slice(0, 10) : [];
        merged.commands.permissions[command] = { ...(0, defaults_1.defaultCommandPermission)(), ...merged.commands.permissions[command] };
    }
    for (const module of defaults_1.PROTECTION_MODULES) {
        const fallback = (0, defaults_1.defaultProtection)(module);
    ...
    if ((raw.schemaVersion ?? 0) < 5 && merged.panel.sessionTimeoutSeconds <= 300)
        merged.panel.sessionTimeoutSeconds = 900;
    if ((raw.schemaVersion ?? 0) < 6 && merged.panel.sessionTimeoutSeconds < 1800)
        merged.panel.sessionTimeoutSeconds = 1800;
    merged.community.autoClean.maximumRules = 10;
    merged.schemaVersion = 13;
    merged.updatedAt = new Date().toISOString();
    return merged;
}
const store = new jsonStore_1.JsonStore(id => node_path_1.default.resolve('data', 'guilds', `${id}.json`), defaults_1.createDefaultGuildConfig, migrate);
exports.guildConfigStore = {
    get: (id) => store.get(id),
    set: (id, value) => store.set(id, { ...value, updatedAt: new Date().toISOString() }),
    update: (id, fn) => store.update(id, async (value) => ({ ...(await fn(value)), updatedAt: new Date().toISOString() }))
};
//# sourceMappingURL=guildConfigStore.js.map