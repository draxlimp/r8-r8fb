...
export interface TelloynCanvasInput {
  anonymous: boolean;
  authorName: string;
  ...
  mentionedAvatarUrl: string | null;
  message: string;
  guildName: string;
}
...
export async function createTelloynCard(input: TelloynCanvasInput): Promise<Buffer> {
  ...
  if (!input.anonymous && input.authorAvatarUrl) {
    ...
    if (avatar) drawCircleImage(ctx, avatar, avatarX, avatarY, avatarSize);
    else drawAnonymousAvatar(ctx, avatarX, avatarY, avatarSize);
  } else {
  ...
  if (input.mentionedName) {
    ...
    if (mentionAvatar) drawCircleImage(ctx, mentionAvatar, mentionAvatarX, mentionAvatarY, mentionAvatarSize);
    else drawAnonymousAvatar(ctx, mentionAvatarX, mentionAvatarY, mentionAvatarSize);
...
async function safeLoadImage(url: string): Promise<any | null> {
  try { return await loadImage(url); }
  catch { return null; }
}

function drawCircleImage(ctx: any, image: any, x: number, y: number, size: number): void {
  ctx.save();
  ctx.beginPath();
...
function drawAnonymousAvatar(ctx: any, x: number, y: number, size: number): void {
  ctx.fillStyle = '#17171a';
  ctx.beginPath();
  ctx.arc(x + size / 2, y + size / 2, size / 2, 0, Math.PI * 2);
  ctx.fill();
