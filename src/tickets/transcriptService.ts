...
export async function createHtmlTranscript(channel: any, ticketId: string, maximumMessages = 10_000): Promise<TranscriptResult> {
  const messages: any[] = [];
  ...
  const directory = path.resolve('data', 'transcripts', channel.guild.id);
  const filePath = path.join(directory, fileName);
  await mkdir(directory, { recursive: true });
  await writeFile(filePath, buildHtml(channel, ticketId, messages), 'utf8');
  return { attachment: new AttachmentBuilder(filePath, { name: fileName }), filePath, messageCount: messages.length };
}

function buildHtml(channel: any, ticketId: string, messages: any[]): string {
  const rows = messages.map(message => {
    ...
    const attachments = [...(message.attachments?.values?.() ?? [])]
      .map((item: any) => `<a href="${escapeAttribute(item.url)}" target="_blank" rel="noreferrer">${escapeHtml(item.name ?? 'anexo')}</a>`)
      .join('<br>');
...
function sanitize(value: string): string { return value.replace(/[^a-z0-9_-]/gi, '-').slice(0, 60); }
